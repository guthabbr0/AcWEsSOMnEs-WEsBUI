...
####################################
# OPEN WEBUI -> AWESOME WEBUI DATA MIGRATION
####################################
# Awesome WebUI specific: optionally copy an existing Open WebUI data
# directory on first run so users switching to Awesome WebUI keep their data.
...
ENABLE_OPEN_WEBUI_DATA_MIGRATION = os.getenv('ENABLE_OPEN_WEBUI_DATA_MIGRATION', 'True').lower() == 'true'
OPEN_WEBUI_DATA_MIGRATION_SOURCE = os.getenv('OPEN_WEBUI_DATA_MIGRATION_SOURCE', '')
OPEN_WEBUI_DATA_MIGRATION_FORCE = os.getenv('OPEN_WEBUI_DATA_MIGRATION_FORCE', 'False').lower() == 'true'
...
if ENABLE_OPEN_WEBUI_DATA_MIGRATION:
    ...
    can_migrate = OPEN_WEBUI_DATA_MIGRATION_FORCE or (
        not destination_has_data and not migration_marker.exists()
    )
    ...
    if can_migrate:
        ...
        for source in source_candidates:
            ...
            try:
                ...
                log.warning(f'Failed to migrate data from {source}: {e}')
...
WEBUI_NAME = os.getenv('WEBUI_NAME', DEFAULT_WEBUI_NAME)
...
WEBUI_GITHUB_REPOSITORY = os.getenv('WEBUI_GITHUB_REPOSITORY', 'mehhovcki-dev/awesome-webui').strip()
if WEBUI_GITHUB_REPOSITORY == '':
    WEBUI_GITHUB_REPOSITORY = 'mehhovcki-dev/awesome-webui'

WEBUI_GITHUB_URL = os.getenv('WEBUI_GITHUB_URL', f'https://github.com/{WEBUI_GITHUB_REPOSITORY}').rstrip('/')

WEBUI_GITHUB_RELEASES_URL = os.getenv('WEBUI_GITHUB_RELEASES_URL', f'{WEBUI_GITHUB_URL}/releases').rstrip('/')

WEBUI_GITHUB_RELEASES_LATEST_API_URL = os.getenv(
