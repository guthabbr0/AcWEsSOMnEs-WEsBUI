...
def upgrade() -> None:
    ...
    if 'model_health_check' not in table_names:
        op.create_table(
            'model_health_check',
    ...
    index_names = {idx['name'] for idx in inspector.get_indexes('model_health_check')}
    if 'ix_model_health_check_model_id' not in index_names:
        op.create_index(
    ...
    if 'ix_model_health_check_checked_at' not in index_names:
        op.create_index(
...
def downgrade() -> None:
    ...
    index_names = {idx['name'] for idx in inspector.get_indexes('model_health_check')}
    if 'ix_model_health_check_checked_at' in index_names:
        op.drop_index('ix_model_health_check_checked_at', table_name='model_health_check')
    if 'ix_model_health_check_model_id' in index_names:
