...
class UserStatus(BaseModel):
    ...
    @classmethod
    def validate_presence_state(cls, value: str | None) -> str | None:
        if value is None:
            return value
...
class UsersTable:
    ...
    async def update_user_oauth_by_id(
        ...
        """Awesome WebUI: add/update an OAuth provider entry in the user's oauth JSON."""
        async with get_async_db_context(db) as session:
            user = await session.get(User, id)
            if not user:
                return None
    ...
    async def delete_user_oauth_by_id(
        self, id: str, provider: str, db: AsyncSession | None = None
    ) -> UserModel | None:
        """Awesome WebUI: remove an OAuth provider entry from the user's oauth JSON."""
        async with get_async_db_context(db) as session:
            user = await session.get(User, id)
            if not user:
                return None
    ...
    @staticmethod
    def normalize_presence_state(value: str | None) -> str:
        """Awesome WebUI: map an arbitrary value to a valid presence state."""
    ...
    @classmethod
    def is_active_for_viewer(cls, user: UserModel, viewer_role: str | None = None) -> bool:
        """Awesome WebUI: active status, hidden for non-admins when user set offline."""
