        ...
        if channel.type == 'dm':
            ...
            users = [
                UserIdNameStatusResponse(
                    **{
                        **u.model_dump(),
                        'is_active': Users.is_active_for_viewer(u, viewer_role=user.role),
                    }
                )
...
async def get_channel_by_id(
    ...
    if channel.type in ['group', 'dm']:
        ...
        write_access = await channel_has_write_access(user.id, channel, db=db)
...
async def get_channel_members_by_id(
    ...
    if channel.type == 'dm':
        ...
        return {
            'users': [UserModelResponse(**u.model_dump(), is_active=Users.is_active_for_viewer(u, viewer_role=user.role)) for u in fetched_users],
            'total': total,
        }
    else:
        filter = {}
...
async def new_message_handler(request: Request, id: str, form_data: MessageForm, user, db):
    ...
    if channel.type in ['group', 'dm']:
        ...
        if user.role != 'admin' and not await channel_has_write_access(user.id, channel, db=db):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=ERROR_MESSAGES.DEFAULT())
...
async def post_new_message(
    ...
    await check_channels_access(request, user)
    if user.role != 'admin':
        channel_ban = await UserModerationBans.get_active_channel_ban(user.id, id, db=db)
        if channel_ban:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=get_moderation_ban_message(channel_ban),
            )
