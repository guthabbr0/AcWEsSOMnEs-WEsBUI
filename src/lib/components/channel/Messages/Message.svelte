<script lang="ts">
	...
	const emojiClusterRegex =
		/(?:\p{Extended_Pictographic}|\p{Emoji_Presentation}|\p{Emoji}\uFE0F)(?:[\uFE0E\uFE0F]|\p{Emoji_Modifier}|\u200D(?:\p{Extended_Pictographic}|\p{Emoji_Presentation}|\p{Emoji}\uFE0F))*/gu;
	...
	const isEmojiOnlyMessage = (content: string) => {
		...
		const withoutShortcodes = trimmed.replace(/:[a-zA-Z0-9_+-]{2,}:/g, '');
		const withoutEmoji = withoutShortcodes.replace(emojiClusterRegex, '');
	...
	const startEdit = async () => {
		...
		await tick();
		document.getElementById(`edit-message-${message.id}`)?.focus();
	};
	...
	const handleMessageKeydown = async (event: KeyboardEvent) => {
		if (disabled || edit || event.defaultPrevented) {
			return;
		}
		...
		if (event.key.toLowerCase() === 'e' && (message.user_id === $user?.id || $user?.role === 'admin')) {
			event.preventDefault();
			await startEdit();
		} else if (event.key === 'Backspace' && (message.user_id === $user?.id || $user?.role === 'admin')) {
			event.preventDefault();
			showDeleteConfirmDialog = true;
		} else if (event.key.toLowerCase() === 'r' && onReply) {
			event.preventDefault();
			onReply(message);
		} else if (event.key.toLowerCase() === 'p' && onPin) {
			event.preventDefault();
			onPin(message);
		} else if (event.key === '+' && onReaction) {
			event.preventDefault();
			showButtons = true;
					...
					{#if edit}
						<div class="py-2">
							<Textarea
								...
								className="bg-gray-100/80 dark:bg-gray-800/70 outline-hidden w-full resize-none rounded px-4 py-3 text-sm"
							...
							/>
							<div class="mt-1.5 mb-1 flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
								<kbd class="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] dark:bg-gray-850">escape</kbd>
								<span>{$i18n.t('to cancel')}</span>
								<span>·</span>
								<kbd class="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] dark:bg-gray-850">enter</kbd>
								<span>{$i18n.t('to save')}</span>
							</div>
						</div>
					{:else}
						<div
							class="min-w-full markdown-prose {pending ? 'opacity-50' : ''} {isEmojiOnlyMessage(
								message.content
							)
								? 'text-4xl leading-tight py-1'
								: ''}"
						>
