<script lang="ts">
	...
	const DEFAULT_PINNED_ITEMS = ['notes', 'workspace', 'model-health'];
	...
	const getPresenceState = () => {
		...
		return presenceOptions.some((option) => option.id === normalized) ? normalized : 'online';
	};
	...
	const updatePresenceState = async (presenceState: PresenceOption['id']) => {
		...
		user.update((currentUser) => ({
			...(currentUser ?? {}),
			...res,
			presence_state: res?.presence_state ?? presenceState
		}));
		show = false;
	};
</script>
				...
				{#if shiftKey}
					...
					>
						<button
							type="button"
							class="p-1 mr-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition"
							on:click|preventDefault|stopPropagation={() => togglePin('model-health')}
						>
							{#if isPinned('model-health')}
								<PinSlash className="size-3.5" strokeWidth="1.5" />
							{:else}
								<Pin className="size-3.5" strokeWidth="1.5" />
							{/if}
